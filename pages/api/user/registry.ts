...
const registryValidator = z.object({
  ...
  name: z
    ...
    .regex(
      /^[^\s].*[^\s]$/,
      "Your name must start and end with non-empty characters"
    ),
  password: z.string(),
  phoneNumber: z
    .string()
    .regex(
      /^[\+\-\s0-9]*$/,
      "Phone number can include +, - and numbers between 0-9"
    ),
...
const RegistryRoute: NextApiHandler = async (req, res) => {
  ...
  if (!fromClient.success) {
    const errorMessages = JSON.parse(fromClient.error.message)
